---
name: mars-unity-game-generator
description: >
    Generate MARS-compliant Unity game-type mini-app (游戏类微应用) packages. Use this skill when the user
    wants to create a game mini-app using Unity engine. Also use when the user mentions "unity",
    "unity-as-a-library", "MarsGameClient", "@mars/sdk/game", "NativeBridgeTransport",
    "appType game" with unity, "cdn-manifest", "game.ts", or asks about game mini-app package structure
    with Unity. This skill covers Unity game-type mini-apps running natively on Expo/React Native via
    unity-as-a-library, with platform service access via MarsGameClient and a TypeScript backend for
    game services (leaderboard, saves, matchmaking).
    For Cocos Creator games, use mars-cocos-game-generator. For app-type mini-apps, use mars-app-generator.
---

# MARS Unity Game Generator

Generate complete, protocol-compliant MARS game-type mini-app packages using **Unity** engine.
This skill encodes the MARS v1.0 protocol standard, game SDK API surface, permission model, and
packaging conventions for Unity games running natively on Expo/React Native.
...
- Unity game frontend import (required):
...
- User wants to **create a Unity game** as a MARS mini-app
- User mentions **Unity**, **unity-as-a-library**
- User asks about **MarsGameClient** or **@mars/sdk/game** usage for Unity
- User wants to **scaffold** a Unity game with leaderboard, saves, or matchmaking
- User asks about **NativeBridgeTransport** or Native Bridge communication
- User asks about **cdn-manifest.json** for Unity game assets

> **Not for app-type!** If the user wants a standard WebView app, use `mars-app-generator`.
> **Not for Cocos!** If the user wants a Cocos Creator game, use `mars-cocos-game-generator`.

## Architecture Overview

Unity game-type mini-apps run **natively** on the Expo/React Native host via
`unity-as-a-library`. The architecture:
...
Key points:
- **Rendering, audio, physics, sensors, haptics** → handled by Unity natively
...
```
my-unity-game/
├── manifest.json          # appType: "game", engine: "unity"
├── permissions.json       # Platform service permissions only
├── cdn-manifest.json      # Large game assets (AssetBundles, etc.)
├── frontend/              # Unity project bridge directory
...
4. The admin page is a regular WebView (not Unity) — use MARS SDK `PostMessageTransport` like an app-type frontend.
...
1. **Game name** — lowercase kebab-case (e.g., `racing-legends`, `tower-defense`)
2. **Game description** — one-sentence summary (Chinese preferred)
3. **Game tags** — up to 5 keywords (e.g., `["游戏", "竞速", "3D"]`)
...
6. **Large assets?** — AssetBundles, 3D models, audio files?
...
>
> 其中 `runtime` 必须为 `"mars@1.0"`，`appType` 必须为 `"game"`，`engine` 必须为 `"unity"`。
...
Rules:
- `appType`: always `"game"`
- `engine`: always `"unity"` for Unity games
...
- `entry.frontend`: directory path — Unity project bridge root
- `webPreview`: (可选) 指向 Unity 的 WebGL 构建产物目录（如 `Build/WebGL` 或 `build/webgl`）。配置后 DevTool 可在 iframe 中直接预览游戏，无需真机
- `capabilities`: ONLY platform service capabilities (`auth`, `storage`, `db`). Do NOT include `gpu`, `audio`, `sensor`, etc. — Unity handles these natively
- Do NOT include `app_id` — platform assigns it
...
|-------|----------------|----------|
| `user.id.read` | false | Player identity |
| `user.profile.read` | true | Player name/avatar |
...
Unity game entry using `MarsGameClient` for platform service access:
...
// Cloud saves
await mars.invoke("save.put", { data: { level: 5, inventory: ["shield", "sword"] } });
...
// === Unity handles everything else ===
// Rendering, audio, physics, input, sensors → Unity native APIs
...
- Rendering, audio, physics, sensors, haptics → Unity APIs directly
...
游戏有大资产时（AssetBundles, 3D 模型, 音频）：
```json
{
    "version": "1",
    "assets": [
        {
            "path": "assets/main.assetbundle",
            "size": 15728640,
            "sha256": "<sha256-hex>",
...
- Unity AssetBundles, 3D models, and large audio go here, NOT in `frontend/`
- Platform uploads to CDN and injects `ctx.env.MARS_CDN_BASE_URL` at runtime

### Step 7: Generate WebGL Build Directory (Build/WebGL/)
...
- 此目录模拟 Unity 的 WebGL 构建产物，但用纯 JS 实现以便在 DevTool iframe 中直接运行
...
- TypeScript for backend and game entry bridge
...
## DevTool WebGL Build Preview

Unity 支持构建为 WebGL 平台，构建产物可在 DevTool 中直接 iframe 预览：

1. 在 Unity Editor 中执行 **File → Build Settings → WebGL → Build**，输出到 `Build/WebGL/`
2. 在 `manifest.json` 中添加 `"webPreview": "Build/WebGL"`
...
> **注意**：WebGL Build 预览仅用于开发调试。生产环境中 Unity 游戏通过 `unity-as-a-library` 原生运行。

> **重要**：生成 Unity 游戏项目时，**必须**在 `manifest.json` 中包含 `"webPreview": "Build/WebGL"` 字段，
> 并创建 `Build/WebGL/` 目录（至少含 `index.html` + 游戏代码），确保 DevTool 可直接预览。
> 否则预览面板会显示占位提示「可在 manifest.json 中添加 webPreview 字段」而非实际游戏画面。

## Common Mistakes to Avoid

1. **Including GPU/audio/sensor capabilities** — Unity handles these natively. Only use `auth`, `storage`, `db` in capabilities
...
7. **Using `entry.frontend` as an HTML file path** — Game frontend entry is a directory
...
11. **Forgetting `webPreview` field in manifest.json** — 生成 Unity 游戏时必须包含 `"webPreview": "Build/WebGL"` 并创建对应的 `Build/WebGL/` 目录（含 `index.html` + 游戏 JS），否则 DevTool 预览面板只显示占位提示
